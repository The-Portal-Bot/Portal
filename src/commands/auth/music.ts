...
export = {
  ...
  async execute(message: Message, args: string[], pGuild: PGuild): Promise<ReturnPromise> {
    return new Promise((resolve) => {
      ...
      const music = message.guild.channels.cache.find((channel) => channel.id == pGuild.musicData.channelId);

      if (music) {
        deleteChannel(PortalChannelTypes.music, <TextChannel>music, message).catch((e) => {
          return resolve({
            result: false,
      ...
      if (args.length === 0) {
            ...
            createMusicLyricsMessage(newMusic, pGuild, musicMessageId)
              ...
              .catch((e) => {
                logger.log({
                  level: 'error',
                  type: 'none',
                  message: new Error(`error creating lyrics message: ${e}`).message,
                });
                return resolve({
                  result: false,
        ...
        if (musicChannel !== '') {
          createMusicChannel(message.guild, musicChannel, musicCategory, pGuild).catch((e) => {
            return resolve({
              result: false,
          ...
          createMusicChannel(message.guild, musicCategory, null, pGuild).catch((e) => {
            return resolve({
              result: false,
              value: `failed to create music channel: ${e}`,
            });
