...
module.exports = async (client, message, args, portal_guilds, portal_managed_guilds_path) => {
	return new Promise((resolve) => {
		...
		for (const portal_id in current_portal_list) {

			let current_portal = current_portal_list[portal_id];
			if (current_portal.voice_list[message.member.voice.channel.id]) {

				const current_voice = current_portal.voice_list[message.member.voice.channel.id];
				if (current_voice.creator_id === message.member.id) {

					const updated_name = guld_mngr.regex_interpreter(
						current_voice.regex,
