      ...
      .then((sentMessage) => {
        ...
        insertPoll(pGuild.id, poll)
          .then((r) => {
            return resolve({
              result: r,
              value: r ? 'successfully created poll' : 'failed to create poll',
            });
          })
          .catch((e) => {
            return resolve({
...
export = {
  ...
  async execute(message: Message, args: string[], pGuild: PGuild): Promise<ReturnPromise> {
    return new Promise((resolve) => {
      ...
      createRoleMessage(<TextChannel>message.channel, pGuild, title, '', '#9900ff', pollMapField, message.author.id)
        .then((r) => {
          return resolve({
            result: r.result,
