...
module.exports = async (args) => {
	...
	if (oldChannel === null && newChannel !== null) { // Joined from null
		...
		if (guld_mngr.included_in_portal_list(newChannel.id, args.guild_list[args.newState.guild.id].portal_list)) { // user joined portal channel
			guld_mngr.create_voice_channel(args.newState, args.guild_list[args.newState.guild.id].portal_list[newChannel.id], newChannel, args.newState.id);
			guld_mngr.generate_channel_name(newChannel, args.guild_list[args.newState.guild.id].portal_list, args.guild_list[args.newState.guild.id]);
		} else if (guld_mngr.included_in_voice_list(newChannel.id, args.guild_list[args.newState.guild.id].portal_list)) { // user joined voice channel
			guld_mngr.generate_channel_name(newChannel, args.guild_list[args.newState.guild.id].portal_list, args.guild_list[args.newState.guild.id]);
		}
	} else if (newChannel === null && oldChannel !== null) { // Left to null
		report_message += 'existing->null\n';

		if (guld_mngr.included_in_portal_list(oldChannel.id, args.guild_list[args.newState.guild.id].portal_list)) { // user left portal channel this part is handled before
		} else if (guld_mngr.included_in_voice_list(oldChannel.id, args.guild_list[args.newState.guild.id].portal_list)) { // user left voice channel
		...
		if (guld_mngr.included_in_portal_list(oldChannel.id, args.guild_list[args.newState.guild.id].portal_list)) {
			report_message += '->source: portal_list\n';

			if (guld_mngr.included_in_portal_list(newChannel.id, args.guild_list[args.newState.guild.id].portal_list)) { // this should not happen
				report_message += '->dest: portal_list\n';
				report_message += 'this should not happen error: portal_channel->portal_channel\n';
			} else if (guld_mngr.included_in_voice_list(newChannel.id, args.guild_list[args.newState.guild.id].portal_list)) { // has been checked before
			...
			if (guld_mngr.included_in_portal_list(newChannel.id, args.guild_list[args.newState.guild.id].portal_list)) { // left created channel and joins portal
				report_message += '->dest: portal_list\n';
				...
				guld_mngr.create_voice_channel(args.newState, args.guild_list[args.newState.guild.id].portal_list[newChannel.id], newChannel, args.newState.id);
				guld_mngr.generate_channel_name(newChannel, args.guild_list[args.newState.guild.id].portal_list, args.guild_list[args.newState.guild.id]);
			} else if (guld_mngr.included_in_voice_list(newChannel.id, args.guild_list[args.newState.guild.id].portal_list)) { // Left created channel and joins another created
				report_message += '->dest: voice_list\n';
			...
			if (guld_mngr.included_in_portal_list(newChannel.id, args.guild_list[args.newState.guild.id].portal_list)) { // Joined portal channel
				report_message += '->dest: portal_list\n';
